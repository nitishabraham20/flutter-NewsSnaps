...
class _NewsScreenState extends ConsumerState<NewsScreen> {
  ...
  Future<void> _handleRefresh() async {
    if (widget.category != null) {
      ref.read(newsProvider.notifier).fetchNews(widget.category!);
    }
  }
  ...
  Widget build(BuildContext context) {
    if (widget.category != null) {
      currentNews = ref.watch(newsProvider);

      Widget? ui;
      if (ref.read(newsProvider.notifier).isLoading) {
        ui = const Center(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (ctx, index) {
              return GestureDetector(
                onTap: () {
                  _onNewsTapped(context, currentNews[index]);
                },
                child: Card(
